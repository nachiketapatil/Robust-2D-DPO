...
def get_hs(split: str, silent: bool = False, cache_dir: str = None):
    print(f'Loading Noisy HelpSteer2D dataset ({split} split)...')
    dataset = pd.read_csv("/nfs/ritik.ritik/Nosiy_2D-DPO/datasets/Noisy_2D_DPO.csv")
    ...
    d["chosen_segment_scores"] = [eval(i) for i in dataset["chosen_segment_scores"]]
    d["rejected_segment_scores"] = [eval(i) for i in dataset["rejected_segment_scores"]]

    final_dataset = pd.DataFrame(d)
    # Convert dataframes to list of dictionaries
    final_dataset_train, final_dataset_eval = train_test_split(final_dataset, train_size=0.8, random_state=0)
    final_dataset_train_list = final_dataset_train.to_dict(orient='records')
    final_dataset_eval_list = final_dataset_eval.to_dict(orient='records')


    if (split == 'train'):
        final_dataset2 = final_dataset_train_list
    else:
        final_dataset2 = final_dataset_eval_list

    dataset = Dataset.from_list(final_dataset2)

    print('done')
    data = defaultdict(lambda: defaultdict(list))
    for row in tqdm.tqdm(dataset, desc='Processing HS', disable=silent):
        prompt = row['prompt']
        chosen = row['chosen']
...
def get_dataset(name: str, split: str, silent: bool = False, cache_dir: str = None):
    ...
    if name == 'shp':
        ...
        data = get_hs(split, silent=silent, cache_dir=cache_dir)
    else:
