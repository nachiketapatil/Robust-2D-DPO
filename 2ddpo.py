...
LR = 5e-7
EVAL_EVERY = 500 #20000
DO_FIRST_EVAL = True
SAMPLE_DURING_EVAL = False #Default True
SEED = 0
BATCH_SIZE = 16#64 #Default 4
EVAL_BATCH_SIZE = 8#32 #Default 16
MAX_LENGTH = 512
...
WANDB_ENABLED = True
WANDB_ENTITY = None
WANDB_PROJECT = "2d-pure-dpo"

DEBUG = False
RUN_DIR = '/nfs/ritik.ritik/Nosiy_2D-DPO/2ddpo_pure_checkpoints'
GRADIENT_ACCUMULATION_STEPS = 2#default 1
MINIMUM_LOG_INTERVAL_SECS = 1.0
MAX_GRAD_NORM = 10.0
EXP_NAME = '2DDPO_pure_pythia69'
LOCAL_DIR = ['.cache']
...
dataset = pd.read_csv("/nfs/ritik.ritik/OML/2D-DPO/HelpSteer_2D_SegScores.csv")    # For Pure Change to noisy later
...
def worker_main(rank: int, world_size: int, policy: nn.Module, reference_model: Optional[nn.Module] = None):
    ...
    trainer = BasicTrainer(policy, SEED, LOCAL_RUN_DIR, reference_model=reference_model, rank=rank, world_size=world_size)
...
def main():
    ...
    worker_main(0, 1, policy, reference_model)
